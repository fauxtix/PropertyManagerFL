...
{
    public class ProprietarioValidator : AbstractValidator<ProprietarioVM>
    {
        public ProprietarioValidator()
        {
            ...
            RuleFor(p => p.DataNascimento)
                .Must(BeAValidDate).WithMessage("Data de Nascimento inválida.(formato: dd/mm/aaaa)")
                .LessThan(x => DateTime.Today)
                .WithMessage("Data de Nascimento inválida (no futuro).");
        }
        ...
        protected bool BeAValidDate(DateTime date)
        {
            ...
            else if ((DateTime.Now.Year - date.Year) > 100)
            {
                result = false;
            }
            else
                result = DataFormat.IsValidDate(date);
        ...
        {
            if (sNIF.Length != 9) return false;
            if(DataFormat.IsNumeric(sNIF) == false)
            {
                return false;
            }
